<template>
    <div
        class="w-[90%] lg:w-[40%] min-h-[30vh] m-8 lg:h-200 rounded-2xl shadow-2xl flex flex-col lg:sticky lg:top-10"
    >
        <!--标题-->
        ...
        <!--表头-->
        <div
            class="grid grid-cols-4 border-b border-b-gray-900"
            style="grid-template-columns: 2fr 1fr 1fr 2fr"
        >
            <p
                v-for="head in table_head"
                :key="head"
        ...
        <div class="grid grid-cols-1 w-full overflow-auto">
            <!--表格内容-->
            <div
                class="grid grid-cols-4 border-b border-b-gray-900 last:border-b-0 hover:font-bold relative"
                style="grid-template-columns: 2fr 1fr 1fr 2fr"
                v-for="transaction_id in sorted_transaction_id"
                :key="transaction_id"
            ...
            >
                <p
                    class="p-1 text-gray-700 break-words text-center"
                    v-for="(data, key) in props.transactions[transaction_id]"
                    :key="key"
                >
...
const table_head = ref([
    t('records_table.headers.time'),
    t('records_table.headers.student'),
    t('records_table.headers.xiuwei'),
    t('records_table.headers.reason'),
]);
