<template>
    <div
        class="w-full min-h-full flex flex-col items-center big-bg"
        v-if="successAPI === successAPITarget"
    >
        <ClassTotal :sum="class_total_score"></ClassTotal>
        <div
            class="flex flex-col lg:flex-row items-center lg:justify-evenly w-full h-auto"
        >
            <div class="flex flex-col w-[90%] lg:w-[65%] mb-4">
                <p class="p-2 text-gray-500">
                    {{ t('individuals.hint') }}
                </p>
                <canvas ref="canvas" class="w-full h-190 shrink-0"></canvas>
            </div>
            <StudentDetail
...
fetch(`${API_route}/ibt?class_id=${class_id}`)
    .then((r) => r.json())
...
fetch(`${API_route}/total_score?class_id=${class_id}`)
    .then((r) => r.json())
