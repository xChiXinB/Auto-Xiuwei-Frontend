    ...
    <div class="w-full min-h-full big-bg" v-if="successAPI === successAPITarget">
        <div class="w-full h-full flex flex-col items-center p-8">
            <div class="w-full overflow-x-auto">
                <table class="w-full border-collapse shadow-2xl rounded-2xl overflow-hidden">
                    <thead>
                        <tr>
                            <th class="bg-400 p-4 text-black font-bold text-center border-r border-r-gray-500">
                                {{ t('rank.table_header.group') }}
                            </th>
                            <th v-for="period_id in sorted_period_ids" :key="period_id"
                    ...
                    <tbody>
                        <tr v-for="group_id in sorted_group_ids" :key="group_id"
                            class="border-b border-b-gray-900 last:border-b-0">
                            <td class="bg-300 p-3 text-black font-bold text-center border-r border-r-gray-500">
                                {{ groups[group_id] }}
                            </td>
                            <td v-for="period_id in sorted_period_ids" :key="period_id"
...
<script setup lang="ts">
    ...
    import { API_route, getPeriods, getGroups } from '../composables/configurations.mts';
    ...
    const sorted_period_ids = computed(() => {
        ...
        return Object.keys(periods.value).map(k => Number(k)).sort((a, b) => order * (a - b));
    });

    // 排序后的组别ID列表
    const sorted_group_ids = computed(() => {
        if (!groups.value) return [];
        return Object.keys(groups.value).map(k => Number(k)).sort((a, b) => a - b);
    });

    // 计算指定组在指定周期的总分
    function getGroupScore(group_id: number, period_id: number): string {
        if (!scores.value || !scores.value[period_id] || !scores.value[period_id][group_id]) {
            return '0.00';
        }
        const group_scores = scores.value[period_id][group_id];
        const total = Object.values(group_scores).reduce((sum: number, score: any) => sum + Number(score), 0);
    ...
    function getRankingsForPeriod(period_id: number): Map<number, number> {
        ...
        for (const gid of sorted_group_ids.value) {
            ...
            const total = group_scores 
                ? Object.values(group_scores).reduce((sum: number, score: any) => sum + Number(score), 0)
                : 0;
    ...
    async function exportToExcel(): Promise<void> {
        ...
        // 构建表格数据
        const data: any[] = [];
        const header = [t('rank.table_header.group'), ...sorted_period_ids.value.map(id => periods.value[id])];
        data.push(header);
